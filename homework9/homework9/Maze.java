...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    vert = verticalSize;
    cell = new DisjointSets(horiz*vert);
     ...
     **/
     ...
     int numOfWalls = (horiz-1)*vert + (vert-1)*horiz;
     int w = numOfWalls;
     int[][] walls = new int[numOfWalls][3];
     int n = 0;
     for (j = 0; j < vert - 1; j++) {
    	 for (i = 0; i < horiz; i++) {
    	      walls[n][0] = i;
     ...
     //put the walls in a random order.
     for(int k=0; k<numOfWalls-1; k++){
    	 int r = randInt(w-1);
    	 swap(walls[r],walls[w-1]);
    	 
    	 w--;
     }
     int i1,i2,j1,j2,c1,c2;
...
//    		 System.out.println("1");
    		 ...
    		 c1 = horiz*j1 + i1;
    		 c2 = horiz*(j1+1) + i1;
//    		 System.out.println(cell.find(c1));
    		 if(!(cell.find(c1) == cell.find(c2))){

...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
	...
	while(node != null){
		int c = key.compareTo(node.entry.key());
		if(c<0){
			node = node.leftChild;
		}else if(c>0){
      ...
      if(find(key) != null){
    	  ...
    	  if( n == root){
    		  if(n.leftChild == null || n.rightChild == null){
    			  ...
    			  BinaryTreeNode x = minNode(n.rightChild);
//    			  System.out.println(x.entry.key());
        		  Entry xe = remove(x.entry.key());
//        		  
        		  n.entry = xe;
    		  }
    		  return n.entry;
    	  ...
    	  size--;
    	  int c = ((Comparable) n.entry.key()).compareTo(n.parent.entry.key());
    	  if(n.leftChild == null && n.rightChild == null){
    		  if(c<0){
